import { NextRequest, NextResponse } from 'next/server';
import { getValidAccessToken } from '@/lib/gsc/google-auth';
import { getGscPropertySnapshot } from '@/lib/gsc/gsc-service';
import { getGscDemoSnapshot } from '@/lib/gsc/demo-data';
import { generateSearchOpportunityGraph } from '@/lib/intelligence/opportunity-graph';

export const runtime = 'nodejs';
export const dynamic = 'force-dynamic';

export async function POST(req: NextRequest) {
  try {
    const body = await req.json();
    const { siteUrl, isDemo, country = 'US', language = 'en' } = body;

    let snapshot;
    if (isDemo || !siteUrl) {
      snapshot = getGscDemoSnapshot();
    } else {
      const token = await getValidAccessToken();
      if (!token) {
        snapshot = getGscDemoSnapshot();
      } else {
        snapshot = await getGscPropertySnapshot(token, siteUrl);
      }
    }

    const graph = await generateSearchOpportunityGraph(
      siteUrl || snapshot.property || 'your-site.com',
      snapshot.queries || [],
      country,
      language
    );

    return NextResponse.json({
      success: true,
      graph,
    });
  } catch (err: unknown) {
    const msg = err instanceof Error ? err.message : 'Failed to generate Search Opportunity Graph';
    return NextResponse.json({ error: msg }, { status: 500 });
  }
}
